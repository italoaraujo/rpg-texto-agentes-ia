# Arquitetura da Pipeline LangChain (LangChain & Integração DeepSeek)

Este documento define a especificação arquitetural da pipeline de agentes do orquestrador **LangChain (LCEL)**, incluindo seus papéis, objetivos, histórias de fundo (backstories), tarefas sequenciais de turno e como o contexto e a API do **DeepSeek** são injetados na execução do jogo.

---

## 1. Configuração da LLM (DeepSeek & Fallback)

O sistema utiliza a biblioteca `langchain-openai` para interfacear com APIs compatíveis com a OpenAI. O backend FastAPI configura duas instâncias de LLM:

### LLM Primária (DeepSeek)
* **Modelo**: `deepseek-chat` (referência ao modelo `deepseek-v3` ou `deepseek-coder` via API oficial)
* **Configuração de Conexão**:
  ...
  * `max_tokens`: `1500` (ou `2500` para narrativas completas)
  * `timeout`: `4.0` segundos (limite rígido controlado pelo cliente HTTP da LLM).

### LLM Reserva (Fallback / Contingência)
* **Modelo**: `gpt-4o-mini` (ou outro modelo rápido e econômico)
* **Configuração de Conexão**:
...
## 2. Personas e Prompts do Sistema (Game Master & NPC)

A pipeline é composta por dois papéis principais que colaboram de maneira sequencial e coordenada para resolver cada turno do jogador.

### A. Agente Mestre (Game Master - GM)
* **Role**: `Diretor de Jogo e Narrador Principal`
* **Goal**: `Arbitrar as ações do jogador de forma imparcial de acordo com as regras de RPG clássicas, descrever as consequências físicas no mundo e atualizar o estado de vida, inventário e habilidades do jogador.`
...
* **System Prompts Suportados**:
  ...
  * **Lyra (Maga Élfica Racional)**: Acadêmica eloquente e lógica, curiosa sobre forças arcanas e feitiços, com um leve toque de superioridade intelectual.
  * **Nenhum (Eco do Ambiente)**: Se o jogador optar por viajar sozinho, este papel assume a persona "Eco do Ambiente e Sussurro das Sombras", narrando presságios, calafrios e a tensão psicológica da solidão absoluta.
* **LLM Ativa**: Vinculada à LLM Primária (DeepSeek) com chave de fallback ativo.

---

## 3. Tarefas Sequenciais do Turno (Pipeline LangChain Expression Language - LCEL)

O loop de jogo executa um processo sequencial de três tarefas encadeadas através de `ChatPromptTemplate` do LangChain a cada ação submetida pelo jogador.
...
* **Descrição da Tarefa**:
  ...
  * Se o jogador for atingido, calcule o dano (um número inteiro). Se ele usar um item de cura do inventário, remova-o do inventário e determine a cura. A pipeline também pode narrar e conceder novas habilidades (aprendizado por eventos/tomos) ou remover habilidades (esquecimento/maldições).
  * Retorne uma descrição factual das consequências físicas no ambiente.
* **Estrutura de Saída**: Texto narrativo físico factual.

### Tarefa 2: Reação do Companheiro (Diálogo)
* **Agente Responsável**: Agente NPC / Eco do Ambiente
* **Entradas (Contexto Injetado)**:
  * Saída da Tarefa 1 (Resolução de Ação e Mecânica).
* **Descrição da Tarefa**:
  > Leia o resultado físico determinado pelo Mestre na Tarefa anterior. Como o personagem companheiro ativo, reaja de forma falada e corporal a esses acontecimentos.
...
### Tarefa 3: Consolidação e Formatação da Narrativa (JSON)
* **Agente Responsável**: Agente Mestre
* **Entradas (Contexto Injetado)**:
  * Saída da Tarefa 1 (Resolução física).
  * Saída da Tarefa 2 (Reação do NPC).
...
* **Estrutura de Saída (JSON Final)**: O orquestrador estruturará a resposta JSON contendo:
  ```json
  {
    ...
    "companions": ["Eldon"],
    "skills": ["Golpe Poderoso", "Bloqueio com Escudo"]
  }
  ```
...
graph TD
    ...
    FetchMemory --> PrepareParams[Monta Prompts no LangChain]
    ...
    ResponseSuccess --> AssembleState[Consolida Estado e Narrativa via LCEL]
...
* **Estrutura de Memória**: O histórico de turnos anteriores e o estado absoluto do jogador são injetados nas tarefas do LangChain.
* **Consistência de Estado**: O estado do jogador (vida, inventário, ambiente, companheiros, habilidades) é o contrato mestre. Se a LLM tentar adicionar itens ou alterar a vida de forma desconectada das mecânicas, o parser do backend valida e ajusta os valores antes de responder ao Frontend.
